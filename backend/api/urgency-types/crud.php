...
try {
    ...
    if ($method === 'POST') {
        $name = clean_string($payload['name'] ?? '', 100);
        $description = clean_string($payload['description'] ?? '', 255);
        if ($name === '') {
            json_response('error', 'Name is required.', null, 422);
        }

        $id = $repo->createOrReactivate('urgency_types', $name, $description === '' ? null : $description);
        json_response('success', 'Urgency type saved.', ['id' => $id], 201);
    }
    ...
    if ($method === 'PUT') {
        ...
        if (!$repo->updateOption('urgency_types', $id, $name, $description === '' ? null : $description, $isActive)) {
            json_response('error', 'Urgency type not found.', null, 404);
        }
        json_response('success', 'Urgency type updated.', ['id' => $id]);
    }

    if (!$repo->deactivateOption('urgency_types', $id)) {
        json_response('error', 'Urgency type not found.', null, 404);
    }
    json_response('success', 'Urgency type deactivated.', ['id' => $id]);
} catch (PDOException $exception) {
    if ($exception->getCode() === '23000') {
        json_response('error', 'An option with this name already exists.', null, 409);
    }
    json_response('error', 'Unable to process urgency type request.', null, 500);
} catch (Throwable) {
    json_response('error', 'Unable to process urgency type request.', null, 500);
}
