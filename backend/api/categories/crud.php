...
try {
    ...
    if ($method === 'POST') {
        ...
        $id = $repo->createOrReactivate('categories', $name, $description === '' ? null : $description);
        json_response('success', 'Category saved.', ['id' => $id], 201);
    }
    ...
    if ($method === 'PUT') {
        ...
        if (!$repo->updateOption('categories', $id, $name, $description === '' ? null : $description, $isActive)) {
            json_response('error', 'Category not found.', null, 404);
        }
        json_response('success', 'Category updated.', ['id' => $id]);
    }

    if (!$repo->deactivateOption('categories', $id)) {
        json_response('error', 'Category not found.', null, 404);
    }
    json_response('success', 'Category deactivated.', ['id' => $id]);
} catch (PDOException $exception) {
    if ($exception->getCode() === '23000') {
        json_response('error', 'An option with this name already exists.', null, 409);
    }
    json_response('error', 'Unable to process category request.', null, 500);
} catch (Throwable) {
    json_response('error', 'Unable to process category request.', null, 500);
}
